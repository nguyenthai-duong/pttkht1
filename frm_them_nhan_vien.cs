    ...
    public partial class frm_them_nhan_vien : Form
    {
        ...
        private void ButtonConfirmNewStaff_Click(object sender, EventArgs e)
        {
            ...
            if ((result == DialogResult.Yes) && (pictureBox1.Image != null))
            {
                ...
                string genre = radioButtonMale.Checked ? genre1 : genre2;

                string insertQuery = "INSERT INTO `staff` (`staff_id`, `staff_name`, `gender`, `dob`, `phone`, `email`) VALUES (@randomCode, @staffName, @genre, @dateString, @staffPhoneNumber, @staffEmail)";
                ...
                insertQuerycmd.Parameters.AddWithValue("@genre", genre);
                insertQuerycmd.Parameters.AddWithValue("@dateString", dateString);
                insertQuerycmd.Parameters.AddWithValue("@staffPhoneNumber", staffPhoneNumber);
                insertQuerycmd.Parameters.AddWithValue("@staffEmail", email);
            ...
            else if ((result == DialogResult.Yes) && ((string.IsNullOrEmpty(textBox1.Text)) || (string.IsNullOrEmpty(textBox4.Text))))
            {
